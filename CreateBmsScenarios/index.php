<?php
    ...
    $allDevices = getBmsDevices($_SESSION['behamad_uuid'],$_SESSION['behamad_token']) -> data;
...
<body>
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="content">
            ...
            <div class="container pt-4 px-4">
                <div class="row bg-dark rounded align-items-start justify-content-center" id="buildingList">
                    <div class="col-sm-6 col-xl-6 section">
                            <div class="bg-secondary rounded d-flex-column align-items-center justify-content-center p-4" style="text-align:center">
                                <h4>دستگاه مبدا</h4>
                                <select class="form-select form-select-lg mb-3 select-device-drop" aria-label=".form-select-sm example" id="bmsScenarioSource">
                                    <option selected>انتخاب کنید</option>
                                    <?php foreach($allDevices as $device): ?>
                                        <option value=<?= $device->deviceid ?> appid=<?= $device->deviceappid ?> eui=<?= $device -> deviceeui ?>> <?= $device -> devicename ?> </option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                    </div>
                    <div class="col-sm-6 col-xl-6 section">
                            <div class="bg-secondary rounded d-flex-column align-items-center justify-content-center p-4" style="text-align:center">
                                <h4>دستگاه مقصد</h4>
                                <select class="form-select form-select-lg mb-3 select-device-drop" aria-label=".form-select-sm example" id="bmsScenarioDestination">
                                    <option selected>انتخاب کنید</option>
                                    <?php foreach($allDevices as $device): ?>
                                        <option value=<?= $device->deviceid ?> type=<?= $device -> devicetype ?> appid=<?= $device->deviceappid ?> eui=<?= $device -> deviceeui ?>> <?= $device -> devicename ?> </option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                    </div>
                </div>
                <div class="row bg-dark rounded align-items-start justify-content-center">
                    <div class="col-sm-12 col-xl-12 section">
                        <div class="bg-secondary rounded d-flex-column align-items-center justify-content-center p-4" style="text-align:center">
                            <button class="btn btn-info btn-block" id="submitBmsScenario">
                                ثبت سناریو
                            </button>
                        </div>
                    </div>
        ...
        <!-- Back to Top -->
        <a href="#" class="btn btn-lg btn-info btn-lg-square back-to-top"><i class="bi bi-plus"></i></a>
    </div>
    ...
    <script src="/js/createBmsScenario.js"></script>
</body>
</html>